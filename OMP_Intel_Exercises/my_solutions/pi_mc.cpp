...
{
   long i;  long Ncirc = 0;
   double pi, x, y, test, runtime;
   double r = 1.0;   // radius of circle. Side of squrare is 2*r 
   
   // TODO: make sure we have numerically correct random algorithm
   // do not want overlaps in random sequences between threads
   // see leap for method in video below
   ...
   seed(-r, r);  // The circle and square are centered at the origin
    ...
    #pragma omp parallel for num_threads(8) private(x,y,test) reduction(+:Ncirc)
    ...
    printf("%ld trials, pi is %lf \n \n",num_trials, pi);
    printf("Runtime with %ld trials = %f1 \n", num_trials, runtime);
